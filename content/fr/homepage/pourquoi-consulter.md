---
title: "🌬️ Pourquoi choisir l’art‑thérapie ?"
description: "Stress, anxiété, hypersensibilité, blocages intérieurs… L'art-thérapie aide à avancer sans tout expliquer. Accompagnement pour adultes et adolescents à Saint-Maximin."
header_menu_title: "Pourquoi consulter ?"
navigation_menu_title: "Pourquoi consulter ?"
weight: 3
header_menu: true
detailed_page_path: /pourquoi-consulter/
detailed_page_homepage_content: true
---


L’art‑thérapie est particulièrement aidante si vous traversez :  
- stress, anxiété, surcharge émotionnelle  
- hypersensibilité ou fatigue mentale  
- difficultés relationnelles  
- perte de sens ou manque d’élan  
- transitions de vie  
- besoin de vous reconnecter à vous‑même  

Aucun talent artistique n’est nécessaire.  
Seule compte votre présence.

{{< icon name="blog">}} [Pourquoi consulter ?](/pourquoi-consulter/)

